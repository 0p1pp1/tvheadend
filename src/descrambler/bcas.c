...
bcas_table_input(void *opaque, int pid, const uint8_t *data, int len, int emm)
{
  ...
  if (ret) {
    if (tvhlog_limit(&bd->err_log, 10))
      tvhinfo(LS_BCAS, "failed to decode ECM. errno:%d", -ret);
    bd->td_keystate = DS_FORBIDDEN;
    return;
  }

  descrambler_keys(opaque, DESCRAMBLER_MULTI2, pid, keys[1], keys[0]);
}
...
bcas_service_stop(th_descrambler_t *td)
{
  ...
  while ((est = SLIST_FIRST(&bd->ecm_streams))) {
    descrambler_close_pid(mux, bd, DESCRAMBLER_ECM_PID(est->pid));
...
bcas_service_start(caclient_t *cac, service_t *s)
{
  ...
  if (!td) {
    ...
    tvhdebug(LS_BCAS, "creating descrambler for svc:%s", service_nicename(s));

    bd = calloc(1, sizeof(*bd));
    if (!bd) {
      tvherror(LS_BCAS, "failed to alloc descrambler for svc:%s",
               service_nicename(s));
      return;
    }
    ...
    snprintf(buf, sizeof(buf), "bcas-%d-%d-%d",
             t->s_dvb_mux->mm_onid, t->s_dvb_mux->mm_tsid,
  ...
  tvhdebug(LS_BCAS, "starting descrambler for svc:%s", service_nicename(s));
  descrambler_change_keystate(td, DS_READY, 1);
  bcas_caid_change(td);
}
...
bcas_create(void)
{
  ...
  if (!cac) {
    tvherror(LS_BCAS, "failed to alloc caclient.");
    return NULL;
  }
